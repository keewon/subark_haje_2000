...
Parser::ParseFunc(void)
{
	...
	isProc = strcmp(szToken, "function") != 0;
	...
	} else funcF->lpslDtLocal = new Scope();
...
Parser::FetchNextToken(void)
{
	...
	while (bComment || cNext=='\0' || cNext=='{' || isspace(cNext)) {
		if (cNext=='{') bComment = true;
		if (cNext=='}') bComment = false;
